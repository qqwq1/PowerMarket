...
import { Factory, TrendingUp, MessageSquare, Package, FileText, ArrowRight } from 'lucide-react'
import { useRouter } from 'next/navigation'
import type { Service, RentalRequest, Stats } from '@/types'
...
function DashboardPage() {
  ...
  const loadDashboardData = async () => {
    try {
      const [statsData, servicesData, requestsData, unreadMessagesCount] = await Promise.all([
        api.get<Stats>('/v1/rentals/stats'),
        api.get<Service[]>('/v1/services'),
        user?.role === 'SUPPLIER'
          ? api.get<RentalRequest[]>('/v1/rental-requests/received')
          : api.get<RentalRequest[]>('/v1/rental-requests/sent'),

        api.get<number>('/v1/chats/unread/count'),
      ])
      setStats({ ...statsData, unreadMessagesCount })
      setRecentServices(servicesData.filter((s) => s.active).slice(0, 5))
      setRecentRequests(requestsData.filter((r) => r.status !== 'COMPLETED').slice(0, 5))
    } catch (error) {
    ...
    <div className="space-y-6">
      ...
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
        ...
        <Card>
          ...
          <CardContent>
            {stats.unreadMessagesCount !== 0 && <div className="text-2xl font-bold">{stats.unreadMessagesCount}</div>}
            <p className="text-xs text-muted-foreground">
              {'Непрочитанных ' + (!stats.unreadMessagesCount && 'сообщений нет')}
            </p>
          </CardContent>
        </Card>
      </div>
          ...
          <CardHeader>
            <div className="flex items-center justify-between">
              ...
              <Button
                variant="ghost"
                size="sm"
                onClick={() => router.push(user?.role === 'SUPPLIER' ? urls.supplier.services : urls.tenant.catalog)}
              >
                Все
                ...
                {recentServices.map((service) => (
                  <div
                    ...
                    onClick={() =>
                      router.push(
                        user?.role === 'SUPPLIER'
                          ? urls.supplier.servicesEdit(service.id)
                          : urls.tenant.catalogServicePage(service.id)
                      )
                    }
                  >
                    ...
                    <div className="text-right">
                      ...
                      <div className="text-sm text-muted-foreground">{service.availableCapacity}</div>
                    </div>
                  </div>
                ))}
              </div>
        ...
        <Card>
          <CardHeader>
            <div className="flex items-center justify-between">
              ...
              <Button variant="ghost" size="sm" onClick={() => router.push(urls.common.rentals)}>
                Все
                <ArrowRight className="w-4 h-4 ml-1" />
              </Button>
            </div>
